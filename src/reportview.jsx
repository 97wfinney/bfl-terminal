...
// A compact award/stat card. `children` MUST be a function: JSX children are
// evaluated at the call site, so passing elements directly would run the body
// even when `show` is false — and the body reads fields off facts that are
// legitimately null when nothing happened that gameweek.
...
// The report renders whatever the pipeline produced; a gap in it should cost
// one page of content, not the whole terminal. Keyed on the selection so
// switching gameweek clears a previous failure.
    ...
    if (this.state.failed) {
      return (
        <div className="panel grow">
          <div className="empty"><b>REPORT ERROR</b><br />this report could not be rendered — try another gameweek</div>
        </div>
      );
    }
